...
for i in range(len(data) - timesteps):
    ...
    y.append(data.values[i + timesteps][1])  # Power Generated is the 2nd column
...
print("Root Mean Squared Error (RMSE):", rmse/70)

# Display the predicted power generation for the first three months of 2024
print("Predicted Power Generation for 2024:")
print(predictions)

# Convert predictions to DataFrame with dates as index
predicted_data = pd.DataFrame(predictions, index=dataset.index[timesteps:timesteps + len(predictions)], columns=["Predicted Power Generation"])
...
predicted_data.to_csv("predicted_power_generation_2024.csv")
