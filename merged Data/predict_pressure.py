...
for i in range(len(data) - timesteps):
    ...
    y.append(data.values[i + timesteps][2])  # pressure is the 3rd column
...
print("Predicted Pressure for 2024:")
print(predictions)

# Convert predictions to DataFrame with dates as index
predicted_data = pd.DataFrame(predictions, index=dataset.index[timesteps:timesteps + len(predictions)], columns=["Predicted Pressure"])
...
predicted_data.to_csv("predicted_pressure_2024.csv")

print("Predicted air temperature for 2024 saved to predicted_pressure_2024.csv file.")
