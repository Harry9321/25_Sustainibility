...
for i in range(len(data) - timesteps):
    ...
    y.append(data.values[i + timesteps][0])  # Air_temperature is the 1st column
...
print("Predicted Air Temperature for 2024:")
print(predictions)

# Convert predictions to DataFrame with dates as index
predicted_data = pd.DataFrame(predictions, index=dataset.index[timesteps:timesteps + len(predictions)], columns=["Predicted Air Temperature"])
...
predicted_data.to_csv("predicted_air_temperature_2024.csv")

print("Predicted air temperature for 2024 saved to predicted_air_temperature_2024.csv file.")
